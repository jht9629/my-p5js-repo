function setup() {
  ...
  // Need to draw in setup to avoid blank frame
  // frameCount === 0 at this point
  draw_step();
}

function draw() {
  // When first draw called frameCount == 1
  draw_step();
}
...
function draw_step() {
  ...
  const frame_index = frameCount % 3;
...
function report_msg(a_msg) {
  ...
  let a_prefix = 'frameCount ' + frameCount;
  a_prefix += ' frameRate ' + frame_rate;
