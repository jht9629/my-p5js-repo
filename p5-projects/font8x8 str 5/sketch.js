...
const start_index = 0;
const end_index = draw_str.length - 1;
...
const pause_page_count = 4;
let pause_page_index;

let draw_op;

function setup() {
  createCanvas(80 * 6, 80 * 3);
  begin_page();
  frameRate(10);
  draw_func = draw_next_char;
}
...
// Pause on a black page for pause_page_count frames
function pause_page_setup() {
  background(0);
  pause_page_index = pause_page_count;
  draw_func = pause_page_draw;
}

function pause_page_draw() {
  pause_page_index -= 1;
  if (pause_page_index < 0) draw_func = draw_next_char;
}
...
function draw_char(a_char, init_x, init_y) {
  ...
  for (const a_byte of a_bytes) {
    ...
    for (let a_x = 0; a_x < 8; a_x += 1) {
      if (a_byte & (1 << a_x)) {
        rect(init_x + a_x * pix_len, a_y, pix_len, pix_len);
      }
    }
    a_y += pix_len;
  }
