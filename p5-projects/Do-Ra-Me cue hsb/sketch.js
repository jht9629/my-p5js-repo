...
function preload() {
  ...
  a_video = createVideo('zedia/Do-Ra-Mi-trim.mov');
}
...
let c_cue = [
  ...
  { volume: 0.5, cueStart: 0, cueEnd: 5 },
];
let c_index = 0;
...
function show_fft() {
  ...
  for (let i = i_start; i < i_end; i++) {
    ...
    let h_col = map(i, i_start, i_end, 0, 360);
    let s_col = 100; // map(spectrum[i], 0, 255, 50, 100);
    
    fill(h_col, s_col, 100, 120/255);
